...
class Decks {
    ...
    {
        // implement this method!
    	this.saveDecks = new ArrayList<Card>(52);
        
        this.numberDecks= n;
        for (int numberDecks = 1; numberDecks <= this.numberDecks; numberDecks++) {
            for (int suit = 1; suit <= 4; suit++) {
                for (int rank = 1; rank <= 13; rank++) {
                    try {
                        saveDecks.add(new Card(suit, rank));
        ...
        playDecks = new ArrayList<Card>(saveDecks);
    ...
    public void shuffle()
    {
    	Collections.shuffle(this.playDecks);
    	// implement this method!
    }
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException
    {
        ...
        int selectedIndex=0;
        if(numberCards> playDecks.size()){
            throw new PlayingCardException("Not enough cards to deal");
        }
        else{
        selectedIndex = playDecks.size()-1;
        for(int i=0; i<numberCards; i++){
            
            hand.add(playDecks.remove(selectedIndex));
            selectedIndex--;
        }
        }
        return hand;
